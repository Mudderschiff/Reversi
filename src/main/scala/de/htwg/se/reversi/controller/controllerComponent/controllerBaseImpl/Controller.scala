...
class Controller @Inject()(var grid: GridInterface) extends ControllerInterface with Publisher {
  ...
  val groute: Route = get {
    pathSingleSlash {
      complete(HttpEntity(ContentTypes.`text/plain(UTF-8)`, "<h1>Controller</h1>"))
    }
    path("controller") {
      complete(HttpEntity(ContentTypes.`text/html(UTF-8)`, "<h1>Controller</h1>"))
      //gridtoHtml("xD")
    } ~
  ...
  def gridtoHtml(xd: String): StandardRoute = {
    complete(HttpEntity(ContentTypes.`text/html(UTF-8)`, "<h1>Controller</h1>" + "<p>"+xd+"</p>"))
  }
